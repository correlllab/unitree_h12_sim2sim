...
from sim2mujoco.utils import quat_rotate_inverse
...
class ObservationProcessor:
    ...
    def _create_term(self, name: str, config: dict, joint_names: list[str], device: torch.device) -> ObservationTerm:
        ...
        elif name == "generated_commands":
            term.compute_raw_fn = lambda sim_state: self._compute_velocity_command()
        elif name == "velocity_and_height_command":
    ...
    def _compute_velocity_command(self) -> torch.Tensor:
        """3D command: [lin_vel_x, lin_vel_y, ang_vel_z]."""
        if self.command_manager is not None:
            return self.command_manager.get_navigation_command()
        # Fallback to zeros (backward compatibility)
        return torch.zeros(3, device=self.device, dtype=torch.float32)
    ...
    def _compute_velocity_height_command(self) -> torch.Tensor:
        ...
        return torch.tensor([0.0, 0.0, 0.0, 1.03], device=self.device, dtype=torch.float32)
