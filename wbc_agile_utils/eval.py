...
# Add command arguments for velocity and height
parser.add_argument(
    "--vel-x",
    type=float,
    default=0.0,
    help="Linear velocity X command (default: 0.0 m/s).",
)
parser.add_argument(
    "--vel-y",
    type=float,
    default=0.0,
    help="Linear velocity Y command (default: 0.0 m/s).",
)
parser.add_argument(
    "--ang-z",
    type=float,
    default=0.0,
    help="Angular velocity Z command (default: 0.0 rad/s).",
)
parser.add_argument(
    "--desired-height",
    type=float,
    default=0.72,
    help="Desired height command (default: 0.72 m).",
)
...
def main():
    ...
    if scheduler:
        ...
        # If no scheduler, set constant commands from CLI arguments
        print(f"[INFO] Using hardcoded commands: vel_x=0.0, vel_y=0.0, ang_z=0.0, height={args_cli.desired_height}")
    ...
    while simulation_app.is_running() and num_steps < args_cli.num_steps:
        ...
        with torch.inference_mode():
            ...
            # Set first 3 observation values to 0 before sending to policy
            ...
            if num_steps % print_interval == 0 and print_count < max_prints:
                print(f"\n{'='*100}")
                print(f"[STEP {num_steps}] - Observation & Action Snapshot (Print {print_count + 1}/{max_prints})")
                print(f"{'='*100}")
                
                # Print full observations for first environment
                if isinstance(obs_tensor, dict):
                    print("[OBSERVATIONS (dict format)]")
                    for key, val in obs_tensor.items():
                        print(f"  {key}: shape={val.shape}")
                        print(f"    Env 0: {val[0].cpu().numpy()}")
                else:
                    print(f"[OBSERVATIONS] shape={obs_tensor.shape}, dtype={obs_tensor.dtype}")
                    obs_np = obs_tensor[0].cpu().numpy().copy()
                    obs_np[0] = 0.0
                    obs_np[1] = 0.0
                    obs_np[2] = 0.0
                    obs_formatted = np.array2string(obs_np, formatter={'float_kind': lambda x: f'{x:.2f}'}, separator=', ')
                    print(f"Env 0 (all {len(obs_np)} dims):")
                    print(f"{obs_formatted}")
                
                # Print actions for all environments
                ...
                print(f"Env 0 actions: {actions_formatted}")
