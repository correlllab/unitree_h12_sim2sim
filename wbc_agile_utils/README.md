...
This directory contains all scripts for training, evaluation, and system management for the Unitree H12 Sim2Sim project.

*Adapted from: [NVIDIA Isaac WBC-AGILE Scripts](https://github.com/nvidia-isaac/WBC-AGILE/tree/main/scripts)*
...
1. **Export Policy to TorchScript:**
   ```bash
   python scripts/eval.py --task Unitree-H12-Stand-v0 --checkpoint path/to/checkpoint.pt
   # This automatically exports policy.pt in the checkpoint directory's exported/ folder
   ```

2. **Export I/O Descriptors:**
   ```bash
   python scripts/export_IODescriptors.py --task Unitree-H12-Stand-v0 --output_dir path/to/output
...
Example usage:
```bash
python scripts/export_IODescriptors.py --task Unitree-H12-Stand-v0 --output_dir .
```
